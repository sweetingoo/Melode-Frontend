...
const Dashboard = () => {
  ...
  const shouldFetchDashboard = isClient && (hasWildcardPermissions.rolePermissions || hasWildcardPermissions.directPermissions);
  ...
  }, [currentUserPermissions, currentUserDirectPermissions, hasWildcardPermissions]);

  // Check specific permissions
  const canManageUsers = userPermissionNames.includes('*') || userPermissionNames.some(perm => 
    perm.includes('users') || perm === 'users:read' || perm === 'users:write'
  );
  const canManageRoles = userPermissionNames.includes('*') || userPermissionNames.some(perm => 
    perm.includes('roles') || perm === 'roles:read' || perm === 'roles:write'
  );
  const canManagePermissions = userPermissionNames.includes('*') || userPermissionNames.some(perm => 
    perm.includes('permissions') || perm === 'permissions:read' || perm === 'permissions:write'
  );
  const canCreateInvitations = userPermissionNames.includes('*') || userPermissionNames.some(perm => 
    perm.includes('invitations') || perm === 'invitations:create' || perm === 'invitations:write'
  );
  ...
  const getTrendIcon = (changePercentage) => {
    if (changePercentage > 0) return <TrendingUp className="h-3 w-3 text-green-600" />;
    if (changePercentage < 0) return <TrendingDown className="h-3 w-3 text-red-600" />;
  ...
  const getHealthStatus = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case "warning":
      case "degraded":
        return { color: "text-yellow-600", icon: AlertCircle, bg: "bg-yellow-100" };
      case "error":
      case "down":
        return { color: "text-red-600", icon: AlertCircle, bg: "bg-red-100" };
      default:
        return { color: "text-muted-foreground", icon: Activity, bg: "bg-muted" };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {shouldFetchDashboard && (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                ...
                <span className={getTrendColor(dashboardData.total_users?.change_percentage)}>
                  {dashboardData.total_users?.change_text || "No change"}
                </span>
                <span>from last {period}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {dashboardData.total_users?.description || "Active users in the system"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {dashboardData.new_invitations?.value?.toLocaleString() || "0"}
              </div>
              <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                {getTrendIcon(dashboardData.new_invitations?.change_percentage)}
                <span className={getTrendColor(dashboardData.new_invitations?.change_percentage)}>
                  {dashboardData.new_invitations?.change_text || "No change"}
                </span>
                <span>from last {period}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {dashboardData.new_invitations?.description || "Invitations sent this period"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                {getTrendIcon(dashboardData.active_roles?.change_percentage)}
                <span className={getTrendColor(dashboardData.active_roles?.change_percentage)}>
                  {dashboardData.active_roles?.change_text || "No change"}
                </span>
                <span>from last {period}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {dashboardData.active_roles?.description || "Roles currently in use"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                ...
                <span className={getTrendColor(dashboardData.permissions?.change_percentage)}>
                  {dashboardData.permissions?.change_text || "No change"}
                </span>
                <span>from last {period}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {dashboardData.permissions?.description || "Total permission sets"}
              </p>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">
                  {dashboardData?.system_activity?.user_registrations?.toLocaleString() || "0"} this {period}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Active Sessions</span>
                <span className="text-sm text-muted-foreground">
                  {dashboardData?.system_activity?.active_sessions?.toLocaleString() || "0"} online
                </span>
              </div>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {dashboardData?.system_health && (
                    <>
                      <Badge 
                        variant="outline" 
                        className={`${getHealthStatus(dashboardData.system_health.status).color} border-current`}
                      >
                        {dashboardData.system_health.status}
                      </Badge>
                      <span className="text-sm font-medium">
                        {dashboardData.system_health.uptime_percentage?.toFixed(1) || "0"}% uptime
                      </span>
                    </>
                  )}
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4 my-4">
              ...
              {!canCreateInvitations && !canManageUsers && !canManageRoles && !canManagePermissions && (
                <div className="text-center text-muted-foreground py-8">
                  <AlertCircle className="h-8 w-8 mx-auto mb-2" />
                  <p className="text-sm">No actions available with your current permissions.</p>
                </div>
              )}
            </div>
          </CardContent>
      ...
      {!shouldFetchDashboard && currentUserData && (
        <div className="mt-8">
          <div className="mb-6">
            <h2 className="text-2xl font-bold tracking-tight">Your Permissions</h2>
            <p className="text-muted-foreground">
            ...
            <Card className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950">
              ...
              <CardContent>
                <p className="text-amber-700 dark:text-amber-300">
                  You have superuser privileges with access to all system permissions and features.
                </p>
              </CardContent>
            </Card>
          ) : (
                ...
                {currentUserDirectPermissions.length > 0 ? (
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-sm">
                            {typeof permission === 'object' 
                              ? permission.display_name || permission.name || permission.slug || permission.permission
                              : permission.replace(':', ': ').replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\b\w/g, l => l.toUpperCase())
                            }
                          </p>
                          ...
                          {typeof permission === 'object' && permission.resource && permission.action && (
                            <p className="text-xs text-muted-foreground mt-1">
                  ...
                  <div className="flex flex-col items-center justify-center py-8">
                    ...
                    <p className="text-muted-foreground text-center">
                      You don't have any specific permissions assigned. Contact your administrator for access.
                    </p>
                  </div>
                )}
              </CardContent>
