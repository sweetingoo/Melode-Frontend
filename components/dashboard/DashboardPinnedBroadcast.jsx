...
export function DashboardPinnedBroadcast() {
  ...
  const card = (
    <Card
      className={`rounded-2xl border-0 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent shadow-sm transition-all duration-200 overflow-hidden ${
        broadcast.slug ? "hover:shadow-md" : "opacity-80"
      }`}
    >
  ...
  if (!broadcast.slug) return card;

  return <Link href={`/admin/broadcasts/${broadcast.slug}`}>{card}</Link>;
}
